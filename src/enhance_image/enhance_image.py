...
def enhance_single_image(model, input_path, output_path, patch_size=128):
    ...
    sr_image = sr_image.resize(
        (original_size[0] * 4, original_size[1] * 4), Image.BICUBIC
    )
    print("3")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Улучшение изображения с помощью SRGAN"
    )
    parser.add_argument(
        "--input",
        type=str,
        default="src/inputs/0051x4.png",
        required=True,
        help="Путь к входному изображению",
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Путь для сохранения результата"
    )
    parser.add_argument(
        "--model",
        type=str,
