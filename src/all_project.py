...
def train(epochs, dataloader, initial_epoch=0, checkpoint_path=None):
    ...
    for epoch in range(initial_epoch, initial_epoch + epochs):
        epoch_pbar = tqdm(
            dataloader, desc=f"Epoch {epoch + 1}/{initial_epoch + epochs}", leave=False
        )
        ...
        for i, (low_res, high_res) in enumerate(dataloader):
            ...
            loss_GAN = criterion_GAN(
                pred_fake - pred_real.mean(), torch.ones_like(pred_fake)
            )
            ...
            if i % 10 == 0:
                print(
                    f"[Epoch {epoch}/{initial_epoch + epochs}] [Batch {i}/{len(dataloader)}] "
